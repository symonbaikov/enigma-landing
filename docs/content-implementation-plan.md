...
>
> **Текущий статус (2026-06-26): кодовая миграция и GEO/SEO baseline завершены.** В sitemap сейчас 39 URL: Home, Pricing, Blog index, 5 Product, 3 Solution, 4 Resource hubs, 6 Playbook chapters, 8 Research Lab articles и 10 Blog posts. Home получил видимый GEO answer block; AEO FAQ/глоссарий реализован как `/resources/aeo-faq`; route-level metadata и JSON-LD подключены через `Seo`; `robots.txt`, `sitemap.xml`, canonical и raw Organization/WebSite JSON-LD fallback добавлены. Browser crawl по 39 sitemap URL: 0 issues по title/meta/canonical/H1/schema/image-alt. Открытые вопросы вынесены в `memory/open-loops.md`.
...
- **Лимит исходного Smart Frame плана:** ≤30 структурных страниц. План использует 28; дополнительные blog posts живут отдельно в `/blog/*` и уже отражены в sitemap.
